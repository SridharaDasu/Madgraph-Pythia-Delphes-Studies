...
void LeptonsAnalysis(const char *inputFile)
{
  ...
  *strstr(outputFile, ".root") = '\0';
  ...
  std::array<std::pair<string, string>, 8> histTypes;
  ...
  histTypes[5] = {string("RecEle"), string("Reconstructed Electron")};
  histTypes[6] = {string("RecMuon"), string("Reconstructed Muon")};
  histTypes[7] = {string("RecJet"), string("Reconstructed Jet")};
  ...
  for(Int_t entry = 0; entry < numberOfEntries; ++entry) {
    ...
    int iNPTPlots = NPTPlots;
    for (Int_t i = 0; i < std::min(iNPTPlots, branchElectron->GetEntries()); ++i) {
      string histName = "RecEle" + std::to_string(i)  + std::string("PT");
      Electron* ele = (Electron*) branchElectron->At(i);
      histograms.Fill(histName.c_str(), ele->PT);
    }

    for (Int_t i = 0; i < std::min(iNPTPlots, branchMuon->GetEntries()); ++i) {
      string histName = "RecMuon" + std::to_string(i)  + std::string("PT");
      Muon* muon = (Muon*) branchMuon->At(i);
      histograms.Fill(histName.c_str(), muon->PT);
    }

    for (Int_t i = 0; i < std::min(iNPTPlots, branchJet->GetEntries()); ++i) {
      string histName = "RecJet" + std::to_string(i)  + std::string("PT");
      Jet* jet = (Jet*) branchJet->At(i);
      histograms.Fill(histName.c_str(), jet->PT);
    }
